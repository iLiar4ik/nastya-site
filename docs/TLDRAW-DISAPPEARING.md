...
В **ChunkErrorHandler** вызов **`window.location.reload()` полностью убран**. Обработчики по-прежнему ловят те же ошибки и только пишут в консоль:  
`[ChunkErrorHandler] ChunkLoadError (reload disabled): ...`

- Если после деплоя доска **перестала пропадать** — причина была в этом обработчике.
- Если доска **всё ещё пропадает** — нужно смотреть дальше (логи браузера, вкладка Network на момент исчезновения, другие расширения/скрипты).
