...
6. **Стабильный store** — по результату диагностики (время идёт, пропадает только полотно и UI tldraw) причина внутри tldraw. Добавлено: store создаётся один раз через `createTLStore({ shapeUtils, bindingUtils })` и передаётся в Tldraw как `store={store}`. Так исключается внутреннее пересоздание store и возможный teardown канваса при асинхронной загрузке/персистенции. При кастомном store `persistenceKey` не используется (типы tldraw не допускают оба варианта) — доска пока без автосохранения между сессиями.
