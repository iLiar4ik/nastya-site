...
**Более новая версия и современный UI:** можно собрать фронтенд из официального репозитория Excalidraw вместо образа kiliandeca. В папке есть `Dockerfile.official` и `docker-compose.official.yml` (полный стек: фронт + room + storage + redis). В Dokploy укажите путь к compose **`deploy/excalidraw/docker-compose.official.yml`**. Те же переменные `EXCALIDRAW_ROOM_WS_URL` и `EXCALIDRAW_STORAGE_URL` задайте в Environment; они подставляются при сборке образа. Первая сборка займёт 5–10 минут (клон GitHub, yarn build). После этого не нужны патчи и заглушка Service Worker.

1. **Новое приложение** → тип **Docker Compose**. Источник — репозиторий nastya-site. **Путь к compose:** укажите именно **`deploy/excalidraw/docker-compose.yml`** (с `deploy` без точки в начале). Если указать `.deploy/excalidraw/...`, папки в репо нет и деплой упадёт с «no such file or directory». Сервис **excalidraw** собирается из **Dockerfile.patch** (образ с вшитым entrypoint, который при старте подменяет URL комнат и sw.js) — при первом деплое будет сборка образа. Если сборка падает с «COPY failed» (не найден entrypoint-patch.sh), в Dokploy задайте **Build Context** (или рабочую директорию сборки) равной **`deploy/excalidraw`**, чтобы контекст содержал Dockerfile.patch и entrypoint-patch.sh.
...
- Образ excalidraw собирается из **Dockerfile.patch**: в нём вшит entrypoint (**entrypoint-patch.sh**), который при каждом старте контейнера подменяет в JS адрес `oss-collab.excalidraw.com` на ваш `EXCALIDRAW_ROOM_WS_URL`, подменяет sw.js/service-worker.js на заглушку и создаёт env.json. Так патч не зависит от command в compose (Dokploy может его переопределять). После деплоя сделайте **жёсткое обновление** (Ctrl+Shift+R) или откройте доску в режиме инкогнито.
...
- В логах контейнера **excalidraw-frontend** (Dokploy → приложение → логи) в начале старта должны быть строки вида **«Replacing collaboration URL in JS with: wss://...»** и **«Patched N JS file(s). Starting nginx...»**. Если этих строк нет — возможно, используется старый образ без нашего entrypoint (пересоберите образ: Redeploy с опцией пересборки или задайте Build Context = deploy/excalidraw). Если N = 0, замена URL в бандле не сработала — в DevTools → Network → WS всё ещё будет подключение к `oss-collab.excalidraw.com`. Сделайте жёсткое обновление (Ctrl+Shift+R) или откройте доску в инкогнито.
