...
**Если в Network вообще нет запросов к excalidraw-room (нет socket.io к вашему домену):** Значит доска в iframe грузит **старый бандл**, в котором зашит адрес oss-collab.excalidraw.com — запросы уходят туда (или блокируются). Что сделать: (1) Откройте урок в **режиме инкогнито** (или в другом браузере без кэша) и проверьте Network снова — есть ли запросы к excalidraw-room или к oss-collab. (2) В логах контейнера **excalidraw-frontend** (Dokploy → приложение → логи) найдите строку «Patched N JS file(s)». Если **N = 0**, замена URL в образе не сработала (в этом образе бандл в других путях или без строки oss-collab). После последнего обновления compose при N=0 контейнер дополнительно ищет .js в /usr и /app — если и тогда 0, значит образ kiliandeca/excalidraw собран без этой строки и нужен другой образ или свой билд. (3) Для доски в iframe добавлен параметр ?_cb=2 в URL, чтобы уменьшить кэширование — после деплоя сайта перезайдите в урок и при необходимости откройте доску в инкогнито.
