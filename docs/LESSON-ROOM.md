...
**Если у учителя и ученика при входе через сайт создаётся впечатление, что открывается «разный» Excalidraw** (разный интерфейс, нет синхронизации): оба должны заходить в урок по **одной ссылке** (`/lesson/room/{id}`) — учитель из админки, ученик по своей ссылке на урок. Комната доски одна: `nastya-lesson-{id}`. Если доски всё равно выглядят по-разному или не синхронизируются, чаще всего у кого-то подгружается старая версия из кэша или другой origin. **У обоих** откройте сайт Excalidraw в отдельной вкладке (https://excalidraw.ваш-домен.ru) → F12 → Application → Service Workers → Unregister; затем Storage → Clear site data. После этого снова откройте урок с сайта.
