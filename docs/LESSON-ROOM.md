...
**Если в Network вообще нет запросов к excalidraw-room (нет socket.io к вашему домену):** Значит доска в iframe грузит **старый бандл**, в котором зашит адрес oss-collab.excalidraw.com — запросы уходят туда (или блокируются). Частая причина — **Service Worker** (Workbox): он отдаёт закэшированные чанки, и подмена URL в файлах на сервере до браузера не доходит. В compose при старте контейнера **service-worker.js** подменяется на заглушку, которая при активации снимает регистрацию SW; после деплоя первая загрузка может ещё идти через старый SW (если он уже был в браузере), затем SW снимается и следующие загрузки получают подправленные JS с сервера. Что сделать: (1) Задеплойте обновлённый Excalidraw (compose с отключением SW). (2) Откройте урок в **режиме инкогнито** или в другом браузере и проверьте Network — должны появиться запросы к excalidraw-room. (3) В логах контейнера **excalidraw-frontend** найдите «Patched N JS file(s)» и «Disabling Service Worker». Если N = 0, замена URL не сработала (образ в других путях).
