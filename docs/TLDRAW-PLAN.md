...
**Что уже сделано (production):**
- **Мини-карта отключена** (`components={{ Minimap: null }}`) — она использует WebGL; при потере контекста канвас может пропадать ([issue](https://github.com/tldraw/tldraw/issues/7689)).
- На странице доски внизу слева выводится **«Загружено в HH:mm:ss»** — якорь для диагностики:
  - если **исчезла и эта надпись** — размонтируется вся страница (перезагрузка iframe/страницы или замена React-дерева);
  - если **надпись осталась, а канвас пустой** — проблема внутри tldraw (например, потеря WebGL-контекста).

**В режиме разработки (npm run dev):** у tldraw есть несовместимость с React Strict Mode (двойное монтирование). В проекте включён `reactStrictMode: false`. На production Strict Mode не даёт двойного монта, поэтому отключение важно в первую очередь для dev.
